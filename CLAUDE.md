...
- **POH data is sacred — use it verbatim.** Aircraft performance numbers (cruise, climb, takeoff, landing, anything sourced from a POH) flow through the engine as-published. No interpolation between published cells, no `(weight/gross)²` scaling, no temperature corrections invented to fill a missing column, no extrapolation beyond the chart. When a published axis (weight, pressure altitude, temperature, …) doesn't land exactly on the pilot's request, round UP to the next-higher published cell (heavier / hotter / higher = longer distance, which is the conservative direction for runway fits). If a POH leaves a corner blank (out-of-envelope or balked-landing), drop the corner from the YAML rather than guess. The lookup helpers in `app/src/engine/runway.ts` enforce this; new POH-driven features must too.
